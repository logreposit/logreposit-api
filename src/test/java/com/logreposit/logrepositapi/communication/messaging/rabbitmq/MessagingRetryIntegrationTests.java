...
public class MessagingRetryIntegrationTests
{
    ...
    @Captor
    private ArgumentCaptor<org.springframework.amqp.core.Message> messageCaptor;

    @BeforeEach
    public void setUp() {
        this.amqpAdmin.declareExchange(ExchangeBuilder.fanoutExchange("x.event_integration_test").build());
        this.amqpAdmin.declareBinding(new Binding("q.logreposit_api", Binding.DestinationType.QUEUE, "x.event_integration_test", "", new HashMap<>()));
    ...
    @Test
    public void testRetry_givenMessageWithUnknownType_expectGetsRetried15TimesAndEndsUpInErrorQueue() throws MessagingException
    {
        ...
        await().atMost(15, TimeUnit.SECONDS).untilAsserted(() -> {
            verify(this.rabbitTemplate, times(1)).convertAndSend(eq("x.event_integration_test"), anyString(), any(org.springframework.amqp.core.Message.class));
            verify(this.rabbitTemplate, times(5)).convertAndSend(eq("retry.x.100"), eq("q.logreposit_api"), any(org.springframework.amqp.core.Message.class));
            verify(this.rabbitTemplate, times(5)).convertAndSend(eq("retry.x.200"), eq("q.logreposit_api"), any(org.springframework.amqp.core.Message.class));
            verify(this.rabbitTemplate, times(5)).convertAndSend(eq("retry.x.300"), eq("q.logreposit_api"), any(org.springframework.amqp.core.Message.class));
        ...
        assertThat((Object) capturedMessages.get(0).getMessageProperties().getHeader(MESSAGE_ERROR_COUNT_HEADER_KEY)).isNull();
    ...
    private static void assertErrorCountIs(org.springframework.amqp.core.Message message, Long expectedCount) {
        Object errorCountHeader = message.getMessageProperties().getHeader(MESSAGE_ERROR_COUNT_HEADER_KEY);
