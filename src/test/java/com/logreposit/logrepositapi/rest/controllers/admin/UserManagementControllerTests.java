...
public class UserManagementControllerTests {
  ...
  public void testCreate() throws Exception {
    ...
    String userCreationRequestDtoSerialized =
        this.objectMapper.writeValueAsString(userCreationRequestDto);

    MockHttpServletRequestBuilder request =
        MockMvcRequestBuilders.post("/v1/admin/users")
            .header(
    ...
    Mockito.when(this.userService.create(Mockito.any(User.class)))
        .thenAnswer(
            i -> {
  ...
  public void testCreate_unauthenticated_invalidKey() throws Exception {
    ...
    Mockito.when(this.userService.getByApiKey(Mockito.eq(invalidApiKey)))
  ...
  public void testCreate_alreadyExistent() throws Exception {
    ...
    Mockito.when(this.userService.create(Mockito.any(User.class)))
        .thenThrow(new UserAlreadyExistentException("", userCreationRequestDto.getEmail()));
  ...
  public void testList() throws Exception {
    ...
    Mockito.verify(this.userService, Mockito.times(1))
        .list(pageNumberArgumentCaptor.capture(), pageSizeArgumentCaptor.capture());
  ...
  public void testList_customPaginationSettings() throws Exception {
    ...
    MockHttpServletRequestBuilder request =
        MockMvcRequestBuilders.get("/v1/admin/users?page=" + pageNumber + "&size=" + pageSize)
            .header(
                LogrepositWebMvcConfiguration.API_KEY_HEADER_NAME,
                ControllerTestUtils.ADMIN_USER_API_KEY);
  ...
  public void testCreate_unCompletePayload() throws Exception {
    ...
    requestDtoSerialized =
        requestDtoSerialized.replace(
            requestDtoSerialized.substring(requestDtoSerialized.length() - 1), "");
  ...
  public void testCreate_missingValuesInPayload() throws Exception {
    ...
    this.controller
        ...
        .andExpect(
            jsonPath("$.message")
                .value(
                    "Invalid input data. Field Errors: email -> must not be null (actual value: null) => Please check your input."));
  }
  ...
  public void testCreate_wrongValueTypesInPayload() throws Exception {
    ...
    this.controller
        ...
        .andExpect(
            jsonPath("$.message")
                .value(
                    "Invalid input data. Field Errors: email -> must be a well-formed email address (actual value: 170.25) => Please check your input."));
  }
  ...
  public void testWithWrongHttpRequestMethod_expectError() throws Exception {
    MockHttpServletRequestBuilder request =
        MockMvcRequestBuilders.patch("/v1/admin/users")
            .header(
                LogrepositWebMvcConfiguration.API_KEY_HEADER_NAME,
                ControllerTestUtils.ADMIN_USER_API_KEY);
    ...
    this.controller
        ...
        .andExpect(
            jsonPath("$.message")
                .value(
                    "Given HTTP Method 'PATCH' is not supported on this particular route. Supported HTTP Methods are: GET, POST"));
  }
}
