...
public class RabbitMqMessageRecoverer implements MessageRecoverer {
  ...
  public RabbitMqMessageRecoverer(
  ...
  public void recover(Message message, Throwable throwable) {
    ...
    this.rabbitTemplate.convertAndSend(exchange, consumerQueue, message);

    logger.info(
        "Republished message after {} error(s) to '{}' with routing key '{}' (with new error count {})",
        errorCount,
        exchange,
        consumerQueue,
        newErrorCount);
  ...
  private static long getMessageErrorCount(Message amqpMessage) {
    Object errorCountAsObject =
        amqpMessage.getMessageProperties().getHeaders().get(MESSAGE_ERROR_COUNT_HEADER_KEY);
